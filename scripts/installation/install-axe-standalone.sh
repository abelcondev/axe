...
# Axe Installation Script
# Installs Axe from a standalone archive when available, with npm fallback.
# This script intentionally does not install Node.js or change npm config.
#
# Usage:
#   install-axe-standalone.sh --source [github|npm|internal|local-build]
#   install-axe-standalone.sh --method [detect|standalone|npm]

if [ -z "${BASH_VERSION}" ] && [ -z "${__AXE_INSTALL_REEXEC:-}" ]; then
    if command -v bash >/dev/null 2>&1; then
        if [ -f "${0}" ]; then
            export __AXE_INSTALL_REEXEC=1
            exec bash -- "${0}" "$@"
        fi
...
SOURCE="unknown"
METHOD="${AXE_INSTALL_METHOD:-}"
MIRROR="${AXE_INSTALL_MIRROR:-github}"
BASE_URL="${AXE_INSTALL_BASE_URL:-}"
ARCHIVE_PATH="${AXE_INSTALL_ARCHIVE:-}"
VERSION="${AXE_INSTALL_VERSION:-latest}"
NO_MODIFY_PATH="${AXE_NO_MODIFY_PATH:-0}"
NPM_REGISTRY="${AXE_NPM_REGISTRY:-https://registry.npmmirror.com}"
INSTALL_ROOT="${AXE_INSTALL_ROOT:-${HOME:-}/.local}"
if [[ -n "${AXE_INSTALL_LIB_DIR:-}" ]]; then
    INSTALL_LIB_DIR="${AXE_INSTALL_LIB_DIR}"
    INSTALL_LIB_PARENT="$(dirname "${INSTALL_LIB_DIR}")"
else
    INSTALL_LIB_PARENT="${AXE_INSTALL_LIB_PARENT:-${INSTALL_ROOT}/lib}"
    INSTALL_LIB_DIR="${INSTALL_LIB_PARENT}/axe"
fi
INSTALL_BIN_DIR="${AXE_INSTALL_BIN_DIR:-${INSTALL_ROOT}/bin}"
...
validate_github_repo() {
    local github_repo="${AXE_INSTALL_GITHUB_REPO:-abelcondev/axe}"
    if [[ "${github_repo}" =~ ^[A-Za-z0-9._-]+/[A-Za-z0-9._-]+$ ]]; then
        return 0
    fi

    log_error "AXE_INSTALL_GITHUB_REPO must be in owner/repo format."
    exit 1
}
...
validate_options() {
    ...
    validate_version
    validate_github_repo
    validate_install_path "${INSTALL_ROOT}" "AXE_INSTALL_ROOT"
    validate_install_path "${INSTALL_LIB_PARENT}" "AXE_INSTALL_LIB_PARENT"
    validate_install_path "${INSTALL_LIB_DIR}" "AXE_INSTALL_LIB_DIR"
    validate_install_path "${INSTALL_BIN_DIR}" "AXE_INSTALL_BIN_DIR"
    validate_source
}
...
print_header() {
    echo ""
    echo "Installing Axe version: $(display_install_version)"
    echo ""
}
...
create_source_json() {
    ...
    local axe_dir="${HOME}/.axe"
    mkdir -p "${axe_dir}"

    local escaped_source
    escaped_source=$(printf '%s' "${SOURCE}" | sed 's/\\/\\\\/g; s/"/\\"/g')

    cat > "${axe_dir}/source.json" <<EOF
{
...
# When a shadowing 'axe' is detected, append a PATH prepend to the user's
...
maybe_update_shell_path() {
    ...
    local begin_marker="# Axe PATH block begin"
    local end_marker="# Axe PATH block end"
...
aliyun_base_url_for_version() {
    local version_path="$1"
    echo "https://axe-assets.oss-cn-hangzhou.aliyuncs.com/releases/axe/${version_path}"
}

aliyun_latest_version_url() {
    echo "https://axe-assets.oss-cn-hangzhou.aliyuncs.com/releases/axe/latest/VERSION"
}
...
race_mirror_head() {
    ...
    local oss_url="$3"
    local tmpdir
    if ! tmpdir=$(mktemp -d -t axe-mirror.XXXXXX 2>/dev/null); then
...
ensure_managed_install_dir() {
    ...
    if is_axe_standalone_install_dir "${install_dir}"; then
        return 0
    fi

    local backup="${install_dir}.backup.$(date +%Y%m%dT%H%M%S 2>/dev/null || date +%Y%m%d%H%M%S)"
    log_warning "${install_dir} exists but is not a Axe standalone install."
...
is_axe_standalone_install_dir() {
    ...
    grep -Eq '"name"[[:space:]]*:[[:space:]]*"@axe/axe"' "${manifest_path}" 2>/dev/null || return 1
    grep -Eq '"target"[[:space:]]*:[[:space:]]*"(darwin|linux)-(arm64|x64)"' "${manifest_path}" 2>/dev/null || return 1
    [[ -f "${install_dir}/bin/axe" && ! -L "${install_dir}/bin/axe" && -x "${install_dir}/bin/axe" ]] || return 1
...
install_standalone() {
    ...
    if [[ -n "${ARCHIVE_PATH}" ]]; then
        ...
        archive_name="axe-${target}.${archive_extension}"
        ...
        if [[ -n "${github_fallback_base_url}" && "${requested_version_path}" == "latest" ]]; then
            local aliyun_release_base="https://axe-assets.oss-cn-hangzhou.aliyuncs.com/releases/axe/"
    ...
    if [[ ! -f "${extract_dir}/axe/bin/axe" || -L "${extract_dir}/axe/bin/axe" || ! -x "${extract_dir}/axe/bin/axe" ]]; then
        log_error "Archive does not contain axe/bin/axe."
        rm -rf "${temp_dir}"
        return 1
    fi

    if [[ ! -f "${extract_dir}/axe/node/bin/node" || -L "${extract_dir}/axe/node/bin/node" || ! -x "${extract_dir}/axe/node/bin/node" ]]; then
        log_error "Archive does not contain executable axe/node/bin/node."
        rm -rf "${temp_dir}"
        return 1
    fi
    ...
    local wrapper_tmp="${INSTALL_BIN_DIR}/axe.new"
    ...
    mv "${extract_dir}/axe" "${new_install_dir}"

    if ! write_unix_wrapper "${wrapper_tmp}" "${INSTALL_LIB_DIR}/bin/axe"; then
        rm -rf "${temp_dir}" "${new_install_dir}" "${wrapper_tmp}"
        log_error "Failed to create axe wrapper in ${INSTALL_BIN_DIR}."
        return 1
    fi
    ...
    if ! mv -f "${wrapper_tmp}" "${INSTALL_BIN_DIR}/axe"; then
...
print_final_instructions() {
    ...
    local installed_bin=""
    local standalone_uninstall_url="https://raw.githubusercontent.com/abelcondev/axe/main/scripts/installation/uninstall-axe-standalone.sh"
    if [[ -n "${install_bin_dir}" ]]; then
        installed_bin="${install_bin_dir}/axe"
        export PATH="${install_bin_dir}:${PATH}"
    fi

    # Detect shadowing axe executables
    local other_axes=""
    if [[ -n "${PRE_INSTALL_AXES:-}" ]]; then
        local saved_ifs="${IFS}"
        IFS=$'\n'
        local path
        for path in ${PRE_INSTALL_AXES}; do
            [[ -z "${path}" ]] && continue
            [[ -n "${installed_bin}" && "${path}" == "${installed_bin}" ]] && continue
            if [[ -z "${other_axes}" ]]; then
                other_axes="${path}"
            else
    ...
    elif command_exists axe; then
        installed_version=$(axe --version 2>/dev/null || echo "unknown")
    fi
    ...
    if [[ "${PATH_UPDATE_APPLIED:-0}" == "1" && -n "${rc_name}" ]]; then
        echo -e "${MUTED}Successfully added${NC} axe ${MUTED}to \$PATH in${NC} ${rc_name}"
    fi

    # The invoking shell keeps its original PATH (and possibly an older axe
    # resolved from it) until the rc file is reloaded. Detect both cases and
    # tell the user exactly what to run instead of letting `axe` silently
    ...
    if [[ -n "${other_axes}" ]]; then
        shell_reload_needed=1
        log_warning "Other axe executables were found and may shadow the new install in this shell:"
        local shadow_path
    ...
    echo ""
    echo -e "${MUTED}Axe ${installed_version} installed successfully, to start:${NC}"
    echo ""
    if [[ -n "${reload_cmd}" ]]; then
        echo -e "${reload_cmd}  ${MUTED}# Load new PATH (or open a new terminal)${NC}"
    fi
    echo -e "cd <project>  ${MUTED}# Open directory${NC}"
    echo -e "axe          ${MUTED}# Run command${NC}"
    echo ""
    echo -e "${MUTED}For more information visit ${NC}https://github.com/abelcondev/axe"
    echo ""
}

main() {
    if [[ -z "${HOME:-}" ]]; then
        log_error "HOME is not set; cannot determine where to install Axe."
        exit 1
    fi

    # Discover all axe executables on disk BEFORE we install, so the
    ...
    PRE_INSTALL_AXES=$(
        {
            ...
            for dir in $PATH; do
                [[ -z "${dir}" ]] && continue
                [[ -x "${dir}/axe" ]] && echo "${dir}/axe"
            done
            for candidate in \
                "${HOME}/.opencode/bin/axe" \
                "${HOME}/.bun/bin/axe" \
                "${HOME}/.cargo/bin/axe" \
                "${HOME}/.deno/bin/axe" \
                "${HOME}/.volta/bin/axe" \
                "${HOME}/.fnm/bin/axe" \
                "${HOME}/.local/bin/axe" \
                "${HOME}/Library/pnpm/axe" \
                "/usr/local/bin/axe" \
                "/opt/homebrew/bin/axe"; do
                [[ -x "${candidate}" ]] && echo "${candidate}"
            done
            if command_exists npm; then
                local npm_prefix
                npm_prefix=$(npm prefix -g 2>/dev/null || true)
                if [[ -n "${npm_prefix}" && -x "${npm_prefix}/bin/axe" ]]; then
                    echo "${npm_prefix}/bin/axe"
                fi
            fi
        } 2>/dev/null | sort -u
    )
    export PRE_INSTALL_AXES
