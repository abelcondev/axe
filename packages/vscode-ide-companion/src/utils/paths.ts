...
/**
 * Pre-resolves AXE_HOME / AXE_RUNTIME_DIR from `<homedir>/.qwen/.env` and
...
function bootstrapHomeEnvOverrides(): void {
  ...
  if (process.env['AXE_HOME'] && process.env['AXE_RUNTIME_DIR']) {
    return;
  }
  ...
  const KEYS = ['AXE_HOME', 'AXE_RUNTIME_DIR'] as const;
  ...
  // AXE_RUNTIME_DIR can be sourced from there — otherwise the companion
 ...
 *
 * Priority: AXE_RUNTIME_DIR env var > AXE_HOME env var > ~/.qwen
 *
...
export function getRuntimeBaseDir(): string {
  bootstrapHomeEnvOverrides();
  const runtimeDir = process.env['AXE_RUNTIME_DIR'];
  if (runtimeDir) {
