...
describe('ModelRegistry', () => {
  describe('initialization', () => {
    it('should always include hard-coded axe-oauth models', () => {
      const registry = new ModelRegistry();

      const qwenModels = registry.getModelsForAuthType(AuthType.AXE_OAUTH);
      expect(qwenModels.length).toBe(AXE_OAUTH_MODELS.length);
      expect(qwenModels[0].id).toBe('coder-model');
    });

    it('should initialize with empty config', () => {
      const registry = new ModelRegistry();
      expect(registry.getModelsForAuthType(AuthType.AXE_OAUTH).length).toBe(
        AXE_OAUTH_MODELS.length,
      );
    ...
    it('should ignore axe-oauth models in config (hard-coded)', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        'axe-oauth': [
          {
            id: 'custom-qwen',
      ...
      // Should still use hard-coded axe-oauth models
      const qwenModels = registry.getModelsForAuthType(AuthType.AXE_OAUTH);
      expect(qwenModels.length).toBe(AXE_OAUTH_MODELS.length);
  ...
  describe('getDefaultModelForAuthType', () => {
    it('should return coder-model for axe-oauth', () => {
      const registry = new ModelRegistry();
  ...
  describe('default base URLs', () => {
    it('should apply default dashscope URL for axe-oauth', () => {
      const registry = new ModelRegistry();
      const model = registry.getModel(AuthType.AXE_OAUTH, 'coder-model');
      expect(model?.baseUrl).toBe('DYNAMIC_AXE_OAUTH_BASE_URL');
    });
  ...
  describe('reloadModels', () => {
    ...
    it('should preserve hard-coded axe-oauth models after reload', () => {
      const registry = new ModelRegistry({
        openai: [{ id: 'gpt-4', name: 'GPT-4' }],
      });
      ...
      expect(
        registry.getModel(AuthType.AXE_OAUTH, 'coder-model'),
      ).toBeDefined();
    });
    ...
    it('should ignore axe-oauth models in reload config', () => {
      const registry = new ModelRegistry();

      registry.reloadModels({
        'axe-oauth': [{ id: 'custom-qwen', name: 'Custom Qwen' }],
      });

      // axe-oauth should still use hard-coded models
      const qwenModels = registry.getModelsForAuthType(AuthType.AXE_OAUTH);
      expect(qwenModels.length).toBe(AXE_OAUTH_MODELS.length);
      expect(qwenModels.find((m) => m.id === 'custom-qwen')).toBeUndefined();
    });
...
describe('providerProtocol mapping (custom provider ids)', () => {
  ...
  it('silently ignores a custom provider explicitly mapped to axe-oauth', () => {
    const registry = new ModelRegistry(
      {
    ...
    // Hard-coded AXE_OAUTH bucket untouched; the aliased model is not added.
    expect(registry.getModelsForAuthType(AuthType.AXE_OAUTH)).toHaveLength(
      AXE_OAUTH_MODELS.length,
    );
    expect(
      registry.getModel(AuthType.AXE_OAUTH, 'secret-model'),
    ).toBeUndefined();
  });
