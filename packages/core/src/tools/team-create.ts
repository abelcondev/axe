...
- **Custom agents** defined in \`.axe/agents/\` may have their own tool restrictions. Check their descriptions to understand what they can and cannot do.
...
This creates:
- A team file at \`~/.axe/teams/{team-name}/config.json\`
- A corresponding task list directory at \`~/.axe/tasks/{team-name}/\`
...
- **Team config location**: \`~/.axe/teams/{team-name}/config.json\`
...
\`\`\`
Use the Read tool to read ~/.axe/teams/{team-name}/config.json
\`\`\`

## Task List Coordination

Teams share a task list that all teammates can access at \`~/.axe/tasks/{team-name}/\`.
