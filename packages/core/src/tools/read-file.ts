...
<system-reminder>SDD design gate: this task has user-facing UI and its design is unresolved (\`design: pending\`). Before writing any production code for it, ask the user how to handle design: design the screens together first in the connected design tool (e.g. Penpot via its MCP), pull an existing design they already have (e.g. a Penpot or Figma link via the connected MCP), or skip design. Record the outcome in this task's \`design:\` frontmatter field — a design URL pinned to the specific view (Penpot file URL with page/board, Figma URL with node-id), a YAML list with one URL per view when the task spans several (steps, modals, sheets, states), or \`skipped (user, YYYY-MM-DD)\`. Designs live in the design tool's workspace, not in the repository. Do NOT write production code for this task while \`design:\` is \`pending\`.</system-reminder>`;
