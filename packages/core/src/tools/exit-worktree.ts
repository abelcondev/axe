...
> {
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    ...
    // make exit_worktree look at `<subdir>/.axe/worktrees/<slug>`,
    ...
    //    `.axe/worktrees/` and call `exit_worktree` with a name
...
async function resolveManagedWorktreeFromCwd(
  ...
  while (true) {
    if (
      ...
      path.basename(path.dirname(path.dirname(cursor))) === '.axe'
    ) {
