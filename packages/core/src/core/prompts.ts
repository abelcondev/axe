 ...
 * prompt. The 9-step workflow is always present; the knowledge index is only
...
5. **Choose the path — tasks or direct — and say it out loud** — After archiving, tell the user which entry point the decision warrants and why, and confirm before proceeding. Create \`sdd/tasks/*.md\` files (\`type: Task\`, Gherkin (Given/When/Then) acceptance criteria, explicit dependencies, each referencing its decision) when the decision spans multiple modules or screens, changes the schema/data model, or cannot be implemented and verified in one sitting. Implement directly — appending a line to \`sdd/log.md\` instead — for single-component changes, bug fixes, and cosmetic work (colors, spacing, copy).
