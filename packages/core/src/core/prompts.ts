...
8. **Implement (TDD)** — Write tests first, then the implementation. Keep changes scoped to the agreed task. If an architecture-relevant decision surfaces mid-implementation, record it as a new \`Decision\` before continuing.
9. **Verify** — Run the tests and the app; confirm the acceptance criteria are met before moving on.
