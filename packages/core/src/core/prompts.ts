...
- \`Task\`: \`title, description, decision (decisions/NNN-name.md), tags, status(pending|in-progress|done), timestamp\` + \`# Acceptance criteria\` (Gherkin) / \`# Dependencies\`. UI tasks also carry \`design\`: \`pending\` until the design gate resolves it to a design reference (\`designs/app.pen#frame\`, Figma URL) or \`skipped (user, YYYY-MM-DD)\`.
