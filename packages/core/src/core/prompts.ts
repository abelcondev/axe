...
6. **Design (UI features)** — When the approved work has user-facing UI, ALWAYS ask the user how to handle design before implementing: design the screens first together in the connected design tool (e.g. Penpot via its MCP), implement from an existing design they already have (e.g. a Penpot or Figma link — ask for the URL and pull it via the connected MCP: design context, screenshots, variables), or skip design and implement directly. Ask even if no design tool appears connected, since the user may have one they can enable. Never skip this gate silently because tool discovery came up empty; skipping it is the user's call, not yours. Designs live in the design tool's own workspace (cloud), NOT in the repository — the project references them by URL. The gate is resolved by writing the outcome into the task's \`design:\` frontmatter field: a design URL pinned to the specific view (Penpot: file URL with the page/board; Figma: URL with node-id), or \`skipped (user, YYYY-MM-DD)\` when the user explicitly chooses to skip. A task that spans multiple views — steps, modals, sheets, success/error states — resolves \`design:\` to a YAML list with one URL per view, optionally labeled in parentheses. Never write production code for a task whose \`design:\` is still \`pending\`. When consuming an existing design, treat it as the approved design: reference its URL (file + page/board or node) from the decision (\`resource\`) or its tasks, and match it — spacing, type scale, colors, states — rather than reinterpreting it. Design MCPs typically operate on the file and page currently open/focused in the tool and cannot create files — BEFORE designing, verify what is open via the tool's read operations (e.g. Penpot's \`high_level_overview\`); if nothing or the wrong file is open, ask the user to create/open the project's design file in the tool and confirm before proceeding. Prefer read operations first and describe intended write operations before running them — design MCPs can edit destructively without confirmation. Design iteration is free-form: designs are design artifacts, NOT production code — the approval gate does not apply to them, so iterate with the user as fast as they want. A single shared design file for the whole project is fine: name one top-level board/frame per view (\`feature-view\`, e.g. \`signup-form\`, \`signup-success\`), and when the user approves the design, reference every view the task covers from its \`design:\` field. The implementation must then match the approved design.
...
**Hard rules:**
- **Wait for explicit approval before writing production code.** Discovery, proposals, and tasks come first. Design work in the design tool is not production code — designs may be created and edited freely during the design phase.
...
- \`Task\`: \`title, description, decision (decisions/NNN-name.md), tags, status(pending|in-progress|done), timestamp\` + \`# Acceptance criteria\` (Gherkin) / \`# Dependencies\`. UI tasks also carry \`design\`: \`pending\` until the design gate resolves it to a design URL (Penpot page/board URL, Figma URL with node-id), a YAML list of URLs when the task spans multiple views, or \`skipped (user, YYYY-MM-DD)\`.
