...
Use this when the user wants the loop to work a task list kept in a file (they say "work through my loop.md", "loop over the tasks in .axe/loop.md", or point at such a file). Tasks live in `.axe/loop.md` (project) or `~/.axe/loop.md` (home; project wins). Instead of a natural-language prompt, set the loop's `prompt` to a sentinel so each fire re-reads the file:
